...
app.post("/api/international/certificate/calculate", (req, res) => {
  try {
    ...
    operationOk(res, { ...fidicCore.calculateCertificate(req.body, version.settings), settingsVersion: version.version, settingsSchemaVersion: version.schemaVersion, settingsChecksum: version.checksum, certificateStandard: version.settings.certificateStandard });
  } catch (error) {
    res.status(400).json({ code: 0, msg: error.message, data: null });
  }
});
