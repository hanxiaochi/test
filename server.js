...
const systemBackupService = require("./lib/backup/system-backup");
...
const systemBackupDataDir = path.resolve(process.env.APP_DATA_DIR || dataDir);
...
const systemBackupDir = path.resolve(process.env.APP_SYSTEM_BACKUP_DIR || path.join(systemBackupDataDir, "system-backups"));
...
const workflowModules = regionPackRegistry.workflowModules();
...
function systemBackupPath(fileName) {
  const safeName = path.basename(String(fileName || ""));
  if (!/^system-[0-9TZ.-]+-[a-f0-9]{8}\.zip$/.test(safeName) || safeName !== String(fileName)) throw new Error("Invalid system backup file name");
  const target = path.resolve(systemBackupDir, safeName);
  if (path.dirname(target) !== systemBackupDir || !fs.existsSync(target)) throw new Error("System backup file does not exist");
  return target;
}

function systemBackupRows() {
  return fs.readdirSync(systemBackupDir, { withFileTypes: true })
    .filter((entry) => entry.isFile() && /^system-[0-9TZ.-]+-[a-f0-9]{8}\.zip$/.test(entry.name))
    .map((entry) => {
      const stat = fs.statSync(path.join(systemBackupDir, entry.name));
      return { fileName: entry.name, bytes: stat.size, createdAt: stat.mtime.toISOString() };
    })
    .sort((a, b) => b.createdAt.localeCompare(a.createdAt));
}

function systemBackupAudit(req, action, result, details = {}) {
  authService.store.audit({
    tenantId: req.authUser.tenantId, userId: req.authUser.id, action, result,
    targetType: "system_backup", targetId: String(details.fileName || ""),
    ipAddress: req.ip, userAgent: req.headers["user-agent"], details
  });
}

async function createFullSystemBackup(req) {
  ...
  try {
    ...
    const temporary = `${target}.tmp`;
    const sources = systemBackupService.collectDirectory(systemBackupDataDir, "data", { excludeNames: ["exports", "system-backups"] });
    const bytes = await systemBackupService.createSystemBackup({ sources, createdAt, createdBy: req.authUser.account, applicationVersion: require("./package.json").version });
...
function calculateInternationalCertificate(input) {
  ...
  return {
    ...
    certificateStandard: version.settings.certificateStandard
  };
}
...
function submitInternationalContractEvent(req) {
  ...
  try {
    const settingsVersion = internationalSettingsService.activeSettingsVersion(engine.db);
    const created = internationalContractEvent.createEvent(engine.db, req.body, {
      submittedBy: req.authUser.account,
      submittedByUserId: req.authUser.id,
      settingsVersion: settingsVersion.version,
      settingsSchemaVersion: settingsVersion.schemaVersion,
      settingsChecksum: settingsVersion.checksum,
      noticeRule: settingsVersion.settings.contractEventNoticeRule
    });
    if (created.replay) {
...
function voidInternationalCertificate(req) {
  const previous = engine.db[internationalCertificateRegister.STORAGE_KEY];
  try {
    const record = internationalCertificateRegister.voidCertificate(engine.db, req.params.identifier, {
      reason: req.body.reason,
      voidedBy: req.authUser.account
    });
    const storage = appStore.save(engine.db, {
      actor: req.authUser.account,
      action: `international-certificate:void:${record.certificateNo}`,
      checkpoint: true
    });
...
for (const page of regionPackRegistry.mountedPages()) {
  app[page.method](page.route, (req, res, next) => {
    try {
      html(res, page.render({ req }));
    } catch (error) {
      next(error);
    }
  });
        ...
        <div class="calc-admin-panel">
          ...
          <form id="calc-rules-form">
            ...
            <div class="calc-admin-actions">
              ...
              <button type="button" class="layui-btn layui-btn-primary" onclick="(window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload())">刷新汇总</button>
    ...
    <script>
      (function(){
        ...
        function reloadContent() {
          if (window.appReloadCurrentContent) window.appReloadCurrentContent();
...
function backupManagementHtml(req) {
  ...
  const systemBody = systemBackupRows().map((row) => `<tr>
    <td>${htmlEscape(row.fileName)}</td><td>${htmlEscape(row.createdAt)}</td><td>${(row.bytes / 1024 / 1024).toFixed(2)} MB</td>
    <td class="core-actions"><a href="/api/admin/system_backups/${encodeURIComponent(row.fileName)}/download">下载</a>
    <button type="button" class="layui-btn layui-btn-xs layui-btn-primary" data-verify-system-backup="${htmlEscape(row.fileName)}">验包</button></td>
  </tr>`).join("");
  return `<div class="core-page" data-core-page="backup-management">
    ...
    <div class="core-shell">
      ...
      <div class="core-panel" style="margin-top:12px;overflow:auto;">
        <h3>全系统灾备</h3><div class="core-note">覆盖业务、账号权限、规则、审批、附件和全部租户。恢复仅允许通过离线命令写入新目录。</div>
        <table class="layui-table" lay-size="sm"><thead><tr><th>文件</th><th>创建时间</th><th>归档大小</th><th>操作</th></tr></thead><tbody>${systemBody || '<tr><td colspan="4" class="core-empty">暂无全系统备份</td></tr>'}</tbody></table>
      </div>
    </div>
    <script>(function(){
...
function internationalSettingsHtml(req, options = {}) {
  ...
  const historyRows = history.map((item) => {
    ...
    return `<tr data-settings-version="${item.version}">
      ...
      <td>${item.settings.priceAdjustmentRule && item.settings.priceAdjustmentRule.enabled ? `启用 / ${item.settings.priceAdjustmentRule.components.length}项` : "停用"}</td>
      <td>${item.settings.contractEventNoticeRule && item.settings.contractEventNoticeRule.enabled ? `${item.settings.contractEventNoticeRule.variationNoticeDays} / ${item.settings.contractEventNoticeRule.claimNoticeDays} 天` : "停用"}</td>
  ...
  const contractEventRows = contractEvents.map((item) => {
    ...
    const notice = item.request.noticeAssessment;
    const noticeStatus = notice ? t(`international.notice.${notice.status}`) : "-";
    const noticeTrace = notice ? `${htmlEscape(noticeStatus)}<br>${htmlEscape(notice.deadlineDate || "-")} / ${notice.elapsedDays} ${htmlEscape(t("international.elapsedDays"))}` : "-";
    const actions = [];
    ...
    if (item.states === "已退回" && canSubmit && isOwn) {
      actions.push(`<button type="button" class="layui-btn layui-btn-xs layui-btn-primary replace-international-event" data-id="${htmlEscape(item.id)}" data-no="${htmlEscape(item.eventNo)}" data-type="${htmlEscape(item.request.eventType)}" data-title="${htmlEscape(item.request.title)}" data-occurred-date="${htmlEscape(item.request.occurredDate || item.request.noticeDate)}" data-date="${htmlEscape(item.request.noticeDate)}" data-late-reason="${htmlEscape(item.request.lateNoticeReason || "")}" data-currency="${htmlEscape(item.request.currency)}" data-amount="${htmlEscape(item.request.claimedAmount)}" data-days="${item.request.claimedTimeImpactDays}" data-clause="${htmlEscape(item.request.contractClause)}" data-description="${htmlEscape(item.request.description)}">${htmlEscape(t("international.submitEvent"))}</button>`);
    }
    ...
    return `<tr data-contract-event-id="${htmlEscape(item.id)}">
      <td>${htmlEscape(item.eventNo)}</td><td>${htmlEscape(typeText)}</td><td>${htmlEscape(item.request.title)}</td><td>${htmlEscape(item.request.noticeDate)}</td><td>${noticeTrace}</td>
  ...
  const certificateRows = certificates.map((item) => {
    ...
    return `<tr data-certificate-id="${htmlEscape(item.id)}">
      ...
      <td>${htmlEscape(statusText)}</td><td>${htmlEscape(item.netCertified || "0")} ${htmlEscape(item.baseCurrency)}</td>
      <td>${predecessor}</td><td>V${item.settingsVersion} / S${item.settingsSchemaVersion}</td><td>${htmlEscape(item.issuedBy)}<br>${htmlEscape(item.issuedAt)}</td>
      <td title="${htmlEscape(item.issueChecksum)}"><code>${htmlEscape(item.issueChecksum.slice(0, 12))}</code></td><td>${action}</td>
    </tr>`;
  }).join("");
  ...
  const settingsPanel = manageSettings ? `<form class="core-panel" id="international-settings-form">
    ...
    <fieldset style="border:1px solid #d1d5db;padding:12px;margin:12px 0"><legend>${htmlEscape(t("international.noticeRule"))}</legend>
      <div class="layui-form-item"><label class="layui-form-label" for="contract-event-notice-enabled">${htmlEscape(t("international.noticeEnabled"))}</label><div class="layui-input-block"><input id="contract-event-notice-enabled" type="checkbox"${settings.contractEventNoticeRule.enabled ? " checked" : ""}></div></div>
      <div class="layui-form-item"><label class="layui-form-label" for="variation-notice-days">${htmlEscape(t("international.variationNoticeDays"))}</label><div class="layui-input-block"><input class="layui-input" id="variation-notice-days" type="number" min="1" max="3650" step="1" value="${settings.contractEventNoticeRule.variationNoticeDays}"></div></div>
      <div class="layui-form-item"><label class="layui-form-label" for="claim-notice-days">${htmlEscape(t("international.claimNoticeDays"))}</label><div class="layui-input-block"><input class="layui-input" id="claim-notice-days" type="number" min="1" max="3650" step="1" value="${settings.contractEventNoticeRule.claimNoticeDays}"></div></div>
      <div class="layui-form-item"><label class="layui-form-label" for="late-reason-required">${htmlEscape(t("international.requireLateReason"))}</label><div class="layui-input-block"><input id="late-reason-required" type="checkbox"${settings.contractEventNoticeRule.requireLateReason ? " checked" : ""}></div></div>
    </fieldset>
  ...
  const contractEventPanel = canSubmit ? `<div class="core-panel" style="margin-top:12px;">
    ...
    <div class="layui-form-item"><label class="layui-form-label">${htmlEscape(t("international.occurredDate"))}</label><div class="layui-input-block"><input class="layui-input" id="contract-event-occurred-date" type="date" value="${currentDate}" required></div></div>
    ...
    <div class="layui-form-item"><label class="layui-form-label">${htmlEscape(t("international.lateNoticeReason"))}</label><div class="layui-input-block"><textarea class="layui-textarea" id="contract-event-late-reason" maxlength="1000" rows="2"></textarea></div></div>
  ...
  const historyPanel = manageSettings ? `<div class="core-panel" style="margin-top:12px;overflow:auto;">
    <h3>合同参数版本历史</h3>
    <table class="layui-table" lay-size="sm"><thead><tr><th>版本</th><th>状态</th><th>语言</th><th>基础币种</th><th>证书标准</th><th>指数调价</th><th>${htmlEscape(t("international.noticeRule"))}</th><th>变更原因</th><th>创建人</th><th>创建时间</th><th>最近启用</th><th>校验和</th><th>操作</th></tr></thead><tbody id="international-settings-history">${historyRows}</tbody></table>
  </div>` : "";
  return `<div class="core-page" data-core-page="international-settings" lang="${htmlEscape(settings.locale)}" dir="${htmlEscape(settings.direction)}">
    ...
    <div class="core-shell">
      ...
      <div class="core-grid">
        ...
        <div class="core-panel">
          ...
          <div class="layui-form-item"><label class="layui-form-label">${htmlEscape(t("international.previousRetention"))}</label><div class="layui-input-block"><input class="layui-input" id="fidic-previous-retention" type="number" min="0" step="0.01" value="${htmlEscape(previousRetention)}"></div></div>
          <div class="layui-form-item"><label class="layui-form-label">${htmlEscape(t("international.previousCumulative"))}</label><div class="layui-input-block"><input class="layui-input" id="fidic-previous-cumulative" type="number" min="0" step="0.01" value="${htmlEscape(previousCumulative)}"></div></div>
      ...
      <div class="core-panel" style="margin-top:12px;overflow:auto;">
        ...
        <table class="layui-table" lay-size="sm"><thead><tr><th>${htmlEscape(t("international.eventNo"))}</th><th>${htmlEscape(t("international.eventType"))}</th><th>${htmlEscape(t("international.description"))}</th><th>${htmlEscape(t("international.noticeDate"))}</th><th>${htmlEscape(t("international.noticeTimeliness"))}</th><th>${htmlEscape(t("international.claimedAmount"))}</th><th>${htmlEscape(t("international.approvedAmount"))}</th><th>${htmlEscape(t("international.timeImpact"))}</th><th>${htmlEscape(t("international.applicant"))}</th><th>${htmlEscape(t("international.status"))}</th><th>${htmlEscape(t("international.evidence"))}</th><th>SHA-256</th><th>${htmlEscape(t("international.action"))}</th></tr></thead><tbody id="international-contract-event-register">${contractEventRows || `<tr><td colspan="13" class="core-empty">${htmlEscape(t("international.noEvents"))}</td></tr>`}</tbody></table>
      </div>
      <div class="core-panel" style="margin-top:12px;overflow:auto;">
    ...
    <script>(function(){
      ...
      if(form)form.addEventListener('submit',function(event){event.preventDefault();var data=new FormData(form),payload={changeReason:data.get('changeReason'),locale:data.get('locale'),baseCurrency:data.get('baseCurrency'),certificateStandard:data.get('certificateStandard'),moneyDigits:Number(data.get('moneyDigits')),retentionRate:data.get('retentionRate'),retentionLimitAmount:data.get('retentionLimitAmount'),minimumCertificateAmount:data.get('minimumCertificateAmount'),contractEventNoticeRule:{enabled:root.querySelector('#contract-event-notice-enabled').checked,variationNoticeDays:Number(root.querySelector('#variation-notice-days').value),claimNoticeDays:Number(root.querySelector('#claim-notice-days').value),requireLateReason:root.querySelector('#late-reason-required').checked}};try{payload.exchangeRates=JSON.parse(data.get('exchangeRates')||'{}');payload.currencyDigits=JSON.parse(data.get('currencyDigits')||'{}');payload.priceAdjustmentRule=JSON.parse(data.get('priceAdjustmentRule')||'{}')}catch(error){message.textContent='JSON: '+error.message;return}request('/api/admin/international_settings',payload).then(function(result){message.textContent=${JSON.stringify(t("international.saved"))}+' v'+result.version.version+'，正在刷新...';setTimeout(function(){location.reload()},500)}).catch(function(error){message.textContent=error.message})});
      ...
      if(eventSubmitButton)eventSubmitButton.addEventListener('click',function(){var payload={eventNo:root.querySelector('#contract-event-no').value,supersedesEventId:root.querySelector('#contract-event-supersedes-id').value,eventType:root.querySelector('#contract-event-type').value,title:root.querySelector('#contract-event-title').value,occurredDate:root.querySelector('#contract-event-occurred-date').value,noticeDate:root.querySelector('#contract-event-notice-date').value,lateNoticeReason:root.querySelector('#contract-event-late-reason').value,currency:root.querySelector('#contract-event-currency').value,claimedAmount:root.querySelector('#contract-event-claimed-amount').value,claimedTimeImpactDays:Number(root.querySelector('#contract-event-time-impact').value),contractClause:root.querySelector('#contract-event-clause').value,description:root.querySelector('#contract-event-description').value};if(!payload.eventNo||!payload.title||!payload.occurredDate||!payload.noticeDate){message.textContent=${JSON.stringify(t("international.eventNo"))}+' / '+${JSON.stringify(t("international.description"))}+' / '+${JSON.stringify(t("international.occurredDate"))}+' / '+${JSON.stringify(t("international.noticeDate"))};return}if(!window.confirm(${JSON.stringify(t("international.submitEvent"))}+' '+payload.eventNo+'?'))return;var signature=JSON.stringify(payload);if(!pendingEvent||pendingEvent.signature!==signature)pendingEvent={signature:signature,key:newEventKey()};payload.idempotencyKey=pendingEvent.key;eventSubmitButton.disabled=true;request('/api/international/contract_events',payload).then(function(result){pendingEvent=null;message.textContent=${JSON.stringify(t("international.pending"))}+'：'+result.record.eventNo+' / '+result.record.submissionChecksum.slice(0,12);setTimeout(function(){location.reload()},500)}).catch(function(error){eventSubmitButton.disabled=false;message.textContent=error.message})});
          ...
          Array.prototype.forEach.call(document.querySelectorAll('[data-post]'), function(link) {
            link.onclick = function() {
              fetch(link.getAttribute('data-post'), {
                ...
                setTimeout(function(){ (window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload()); }, 450);
              });
            };
          });
        })();
  ...
  return `
    <div style="padding:16px 20px;">
      <form class="layui-form" id="project-information-hang-form">
        ...
        <div class="layui-form-item">
          <div class="layui-input-block">
            <button type="button" class="layui-btn" onclick="(function(btn){var form=btn.closest('form');var data={};Array.prototype.forEach.call(form.querySelectorAll('[name]'),function(el){data[el.name]=el.value});fetch('/oaDataNode/save_data_node',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(function(r){return r.json()}).then(function(r){if(window.layer){layer.msg(r.msg||'挂接成功')} setTimeout(function(){(window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload())},500)});})(this)">保存挂接</button>
          </div>
        </div>
      </form>
    </div>`;
...
function jlPaymentExportRows(req) {
  ...
  supportReport.jl102Rows.forEach((row) => push("JL102计量支付报表传递单", {
    code: row.formCode,
    name: row.formName,
    source: row.businessNo,
    sectionName: row.sectionName,
    item: row.currentStep,
    amount: row.amount,
    status: row.state,
    result: row.result,
  ...
  supportReport.jl103Rows.forEach((row) => push("JL103施工进度表", {
    code: row.chapter,
  ...
  if (!supportReport.jl108RawMaterialRows.length) push("JL108-1原材料明细表", { name: "本期无原材料调差消耗明细", amount: 0, source: "JL108-1" });
  supportReport.jl108RawMaterialRows.forEach((row) => push("JL108-1原材料明细表", {
    code: row.measureNo,
    name: row.materialName,
    unit: row.unit,
    quantity: row.consumeQuantity,
  ...
  supportReport.jl112Rows.forEach((row) => push("JL112工程量表汇编", {
    code: row.measureNo,
    name: row.position,
    sectionName: row.sectionName,
    quantity: row.detailCount,
    amount: row.amount,
    status: row.state,
    source: row.source,
    formula: row.formula
  }));
  push("JL115开工动员预付款支付证书", {
    code: supportReport.jl115Certificate.formCode,
    name: supportReport.jl115Certificate.formName,
    amount: supportReport.jl115Certificate.periodAdvance,
    contractTotal: supportReport.jl115Certificate.contractTotal,
    advanceRate: supportReport.jl115Certificate.advanceRate,
    cumulativeAmount: supportReport.jl115Certificate.cumulativeAdvance,
    remainingAdvance: supportReport.jl115Certificate.remainingAdvance,
    status: supportReport.jl115Certificate.expected ? "应出现" : "本期可不出现",
    formula: supportReport.jl115Certificate.formula
  });
...
function jlPaymentPrintableHtml(req) {
  ...
  return `
    <div class="jl-print-page">
      ...
      <div class="jl-print-section">
        <h2>JL102/JL103 流转与施工进度</h2>
        <table><thead><tr><th>表号</th><th>单据编号</th><th>合同段/章节</th><th>状态/进度</th><th>金额</th><th>处理人</th><th>时间</th></tr></thead><tbody>
          ${rows(supportReport.jl102Rows.slice(0, 30), (row) => [row.formCode, row.businessNo, row.sectionName, row.result || row.state, moneyText(row.amount), row.approver, row.approveTime]) ||
            `<tr><td colspan="7">本期暂无流转记录</td></tr>`}
          ${rows(supportReport.jl103Rows, (row) => ["JL103", row.chapter, row.chapterName, `${percentText(row.progressPct)} ${row.status}`, moneyText(row.cumulativeAmount), "", ""]) ||
            `<tr><td colspan="7">本期暂无施工进度</td></tr>`}
        </tbody></table>
      </div>
      <div class="jl-print-section">
      ...
      <div class="jl-print-section">
        <h2>JL115 开工动员预付款支付证书</h2>
        <table><thead><tr><th>合同价</th><th>预付款率</th><th>预付款总额</th><th>本期支付</th><th>累计支付</th><th>剩余</th><th>出现状态</th></tr></thead><tbody>
          ${rows([supportReport.jl115Certificate], (row) => [moneyText(row.contractTotal), `${row.advanceRate}%`, moneyText(row.totalAdvance), moneyText(row.periodAdvance), moneyText(row.cumulativeAdvance), moneyText(row.remainingAdvance), row.expected ? "应出现" : "本期可不出现"])}
        </tbody></table>
      </div>
      <div class="jl-print-section">
      ...
      <div class="jl-print-section">
        <h2>JL108-1 原材料明细表</h2>
        <table><thead><tr><th>材料</th><th>单位</th><th>消耗量</th><th>折算系数</th><th>折算数量</th><th>基价</th><th>现行价</th><th>调差金额</th></tr></thead><tbody>
          ${rows(supportReport.jl108RawMaterialRows, (row) => [row.materialName, row.unit, row.consumeQuantity, row.conversionFactor, row.convertedQuantity, moneyText(row.basePrice), moneyText(row.currentPrice), moneyText(row.adjustMoney)]) ||
            `<tr><td colspan="8">本期无原材料调差消耗明细</td></tr>`}
        </tbody></table>
      </div>
      <div class="jl-print-section">
        <h2>JL104 章级汇总</h2>
      ...
      <div class="jl-print-section">
        <h2>JL112 工程量表汇编</h2>
        <table><thead><tr><th>序号</th><th>计量表编号</th><th>合同段</th><th>部位</th><th>明细数</th><th>金额</th><th>状态</th></tr></thead><tbody>
          ${rows(supportReport.jl112Rows, (row) => [row.orderNo, row.measureNo, row.sectionName, row.position, row.detailCount, moneyText(row.amount), row.state]) ||
            `<tr><td colspan="7">本期暂无工程量表汇编</td></tr>`}
        </tbody></table>
      </div>
      <div class="jl-print-section">
...
function jlPaymentReportPageHtml(req) {
  ...
  const jl102Rows = supportReport.jl102Rows.slice(0, 80).map((row) => `
    <tr>
      <td>${htmlEscape(row.formCode)}</td>
      <td>${htmlEscape(row.businessNo)}</td>
      <td class="left">${htmlEscape(row.sectionName || "")}</td>
      <td>${moneyText(row.amount)}</td>
      <td>${htmlEscape(row.state || "")}</td>
      <td>${htmlEscape(row.currentStep || "")}</td>
      <td>${htmlEscape(row.approver || "")}</td>
      <td>${htmlEscape(row.approveTime || "")}</td>
    </tr>`).join("");
  const jl103Rows = supportReport.jl103Rows.map((row) => `
    <tr>
      <td>${htmlEscape(row.chapter)}</td>
      <td class="left">${htmlEscape(row.chapterName)}</td>
      <td>${moneyText(row.contractAmount)}</td>
  ...
  const jl108RawRows = supportReport.jl108RawMaterialRows.map((row) => `
    <tr>
      <td>${htmlEscape(row.materialName || "")}</td>
      <td>${htmlEscape(row.unit || "")}</td>
      <td>${htmlEscape(row.consumeQuantity)}</td>
      <td>${htmlEscape(row.conversionFactor)}</td>
      <td>${htmlEscape(row.convertedQuantity)}</td>
      <td>${moneyText(row.basePrice)}</td>
      <td>${moneyText(row.currentPrice)}</td>
      <td>${moneyText(row.adjustMoney)}</td>
    </tr>`).join("");
  const jl112Rows = supportReport.jl112Rows.map((row) => `
    <tr>
      <td>${htmlEscape(row.orderNo)}</td>
      <td>${htmlEscape(row.measureNo || "")}</td>
      <td class="left">${htmlEscape(row.sectionName || "")}</td>
      <td class="left">${htmlEscape(row.position || "")}</td>
      <td>${htmlEscape(row.detailCount)}</td>
      <td>${moneyText(row.amount)}</td>
      <td>${htmlEscape(row.state || "")}</td>
    </tr>`).join("");
  const jl115Rows = [supportReport.jl115Certificate].map((row) => `
    <tr>
      <td>${moneyText(row.contractTotal)}</td>
      <td>${htmlEscape(row.advanceRate)}%</td>
      <td>${moneyText(row.totalAdvance)}</td>
      <td>${moneyText(row.periodAdvance)}</td>
      <td>${moneyText(row.cumulativeAdvance)}</td>
      <td>${moneyText(row.remainingAdvance)}</td>
      <td>${htmlEscape(row.expected ? "应出现" : "本期可不出现")}</td>
      <td class="left">${htmlEscape(row.formula || "")}</td>
    </tr>`).join("");
      ...
      <div class="core-shell">
        <div class="core-head">
          ...
          <div class="core-tools">
            ...
            <a class="layui-btn layui-btn-sm" href="/api/payment/jl112?periodId=${encodeURIComponent(periodId || "")}&sectionId=${encodeURIComponent(sectionId || "")}">JL112 JSON</a>
            <a class="layui-btn layui-btn-sm" href="/api/payment/jl115?periodId=${encodeURIComponent(periodId || "")}&sectionId=${encodeURIComponent(sectionId || "")}">JL115 JSON</a>
        ...
        <div class="core-grid">
          <div class="core-panel">
            <h3>JL102 计量支付报表传递单 <span class="subtle">审批流转记录</span></h3>
            <table class="layui-table" lay-size="sm">
              <thead><tr><th>表号</th><th>单据编号</th><th>合同段</th><th>金额</th><th>状态</th><th>当前环节</th><th>处理人</th><th>处理时间</th></tr></thead>
              <tbody>${jl102Rows || `<tr><td colspan="8" class="core-empty">本期暂无流转记录</td></tr>`}</tbody>
            </table>
          </div>
          <div class="core-panel">
            <h3>JL103 施工进度表 <span class="subtle">按章节展示累计进度</span></h3>
            <table class="layui-table" lay-size="sm">
              <thead><tr><th>章号</th><th>项目内容</th><th>变更后金额</th><th>本期完成</th><th>累计完成</th><th>进度</th><th>状态</th></tr></thead>
              <tbody>${jl103Rows || `<tr><td colspan="7" class="core-empty">本期暂无施工进度</td></tr>`}</tbody>
            </table>
          </div>
          <div class="core-panel">
            <h3>JL115 开工动员预付款支付证书 <span class="subtle">合同价 × 动员预付款率</span></h3>
            <table class="layui-table" lay-size="sm">
              <thead><tr><th>合同价</th><th>预付款率</th><th>预付款总额</th><th>本期支付</th><th>累计支付</th><th>剩余</th><th>状态</th><th>公式</th></tr></thead>
              <tbody>${jl115Rows}</tbody>
            </table>
          </div>
        </div>
        <div class="core-grid">
          ...
          <div class="core-panel">
            <h3>JL112 工程量表汇编 <span class="subtle">本期JL114计量单汇编封面</span></h3>
            <table class="layui-table" lay-size="sm">
          ...
          <div class="core-panel">
            <h3>JL108-1 原材料明细表 <span class="subtle">消耗量 × 折算系数后进入JL108调差</span></h3>
            <table class="layui-table" lay-size="sm">
              <thead><tr><th>材料</th><th>单位</th><th>消耗量</th><th>折算系数</th><th>折算数量</th><th>基价</th><th>现行价</th><th>调差金额</th></tr></thead>
              <tbody>${jl108RawRows || `<tr><td colspan="8" class="core-empty">本期无原材料调差消耗明细</td></tr>`}</tbody>
            </table>
          </div>
          <div class="core-panel">
            <h3>JL109 材料到场预付</h3>
  ...
  return `
    <div style="padding:16px 20px;">
      <form class="layui-form" id="workflow-adjust-form">
        ...
        <div class="layui-form-item">
          <div class="layui-input-block">
            <button type="button" class="layui-btn" onclick="(function(btn){var form=btn.closest('form');var data={};Array.prototype.forEach.call(form.querySelectorAll('[name]'),function(el){data[el.name]=el.value});fetch('/workflow/adjust_order',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(function(r){return r.json()}).then(function(r){if(window.layer){layer.msg(r.msg||'调整成功')} setTimeout(function(){(window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload())},500)});})(this)">保存调整</button>
          </div>
        </div>
      </form>
    </div>`;
  ...
  return `
    <div style="padding:16px 20px;">
      <form class="layui-form" id="archive-upload-form">
        ...
        <div class="layui-form-item">
          <div class="layui-input-block">
            <button type="button" class="layui-btn" onclick="(function(btn){var form=btn.closest('form');var data={};Array.prototype.forEach.call(form.querySelectorAll('[name]'),function(el){data[el.name]=el.value});fetch('/vary_measure/save_archive_pic',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(function(r){return r.json()}).then(function(r){if(window.layer){layer.msg(r.msg||'归档成功')} setTimeout(function(){(window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload())},500)});})(this)">保存归档</button>
          </div>
        </div>
      </form>
    </div>`;
  ...
  return `
    <div style="padding:16px 20px;">
      <form class="layui-form" id="sec-bill-import-form">
        ...
        <div class="layui-form-item">
          <div class="layui-input-block">
            <button type="button" class="layui-btn" onclick="(function(btn){var form=btn.closest('form');var data={};Array.prototype.forEach.call(form.querySelectorAll('[name]'),function(el){data[el.name]=el.value});fetch('/secBill/import_bill_data',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(data)}).then(function(r){return r.json()}).then(function(r){if(window.layer){layer.msg(r.msg||'导入成功')} setTimeout(function(){(window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload())},500)});})(this)">导入清单</button>
          </div>
        </div>
      </form>
    </div>`;
      ...
      <div class="gather-shell">
        <div class="gather-head">
          ...
          <div class="gather-actions">
            ...
            <button class="layui-btn layui-btn-sm" onclick="fetch('/dataGather/data_collect_gather',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({gatherId:${current.gatherId || 0}})}).then(function(){(window.appReloadCurrentContent?window.appReloadCurrentContent():location.reload())})">采集本期</button>
...
app.get("/api/international/certificates", (_req, res) => {
  try {
    operationOk(res, internationalCertificateRegister.listCertificateSummaries(engine.db, { offset: _req.query.offset, limit: _req.query.limit }));
  } catch (error) {
    res.status(internationalCertificateErrorStatus(error)).json({ code: 0, msg: error.message, data: null });
  }
});
...
app.get("/api/international/certificates/:identifier", (req, res) => {
  try {
    operationOk(res, internationalCertificateRegister.findCertificate(engine.db, req.params.identifier));
  } catch (error) {
    res.status(internationalCertificateErrorStatus(error)).json({ code: 0, msg: error.message, data: null });
  }
});
app.post("/api/international/certificates/issue", (req, res) => {
  try {
    const result = issueInternationalCertificate(req);
    authService.store.audit({
      tenantId: req.authUser.tenantId,
      userId: req.authUser.id,
      action: "international_certificate.issue",
      result: "success",
      targetType: "international_certificate",
      targetId: result.record.id,
      ipAddress: req.ip,
      userAgent: req.headers["user-agent"],
      details: { certificateNo: result.record.certificateNo, replay: result.replay, issueChecksum: result.record.issueChecksum }
    });
    operationOk(res, result);
  } catch (error) {
    authService.store.audit({ tenantId: req.authUser.tenantId, userId: req.authUser.id, action: "international_certificate.issue", result: "failure", targetType: "project", targetId: req.businessContext.projectId, ipAddress: req.ip, userAgent: req.headers["user-agent"], details: { certificateNo: req.body.certificateNo, message: error.message } });
    res.status(internationalCertificateErrorStatus(error)).json({ code: 0, msg: error.message, data: null });
  }
});
app.post("/api/international/certificates/:identifier/void", (req, res) => {
  try {
    const result = voidInternationalCertificate(req);
    authService.store.audit({
      tenantId: req.authUser.tenantId,
      userId: req.authUser.id,
      action: "international_certificate.void",
      result: "success",
      targetType: "international_certificate",
      targetId: result.record.id,
      ipAddress: req.ip,
      userAgent: req.headers["user-agent"],
      details: { certificateNo: result.record.certificateNo, reason: result.record.voidReason, voidChecksum: result.record.voidChecksum }
    });
    operationOk(res, result);
  } catch (error) {
    authService.store.audit({ tenantId: req.authUser.tenantId, userId: req.authUser.id, action: "international_certificate.void", result: "failure", targetType: "international_certificate", targetId: req.params.identifier, ipAddress: req.ip, userAgent: req.headers["user-agent"], details: { reason: req.body.reason, message: error.message } });
    res.status(internationalCertificateErrorStatus(error)).json({ code: 0, msg: error.message, data: null });
  }
});
...
app.post("/api/admin/system_backups", requirePermission("admin:access"), async (req, res) => {
  try {
    ...
    systemBackupAudit(req, "system_backup.create", "success", result);
    operationOk(res, result);
  } catch (error) {
    systemBackupAudit(req, "system_backup.create", "failure", { message: error.message });
    res.status(error.status || 500).json({ code: 0, msg: error.message, data: null });
  }
});
app.post("/api/admin/system_backups/:fileName/verify", requirePermission("admin:access"), async (req, res) => {
  try {
    const source = systemBackupPath(req.params.fileName);
    const verified = await systemBackupService.validateSystemBackup(fs.readFileSync(source));
    const result = { fileName: req.params.fileName, ...verified.manifest.totals, manifestSha256: verified.manifest.manifestSha256 };
    systemBackupAudit(req, "system_backup.verify", "success", result);
    operationOk(res, result);
  } catch (error) {
    systemBackupAudit(req, "system_backup.verify", "failure", { fileName: req.params.fileName, message: error.message });
    res.status(400).json({ code: 0, msg: error.message, data: null });
  }
});
app.get("/api/admin/system_backups/:fileName/download", requirePermission("admin:access"), async (req, res) => {
  try {
    const source = systemBackupPath(req.params.fileName);
    const verified = await systemBackupService.validateSystemBackup(fs.readFileSync(source));
    systemBackupAudit(req, "system_backup.download", "success", { fileName: req.params.fileName, ...verified.manifest.totals });
    res.download(source, req.params.fileName);
  } catch (error) {
    systemBackupAudit(req, "system_backup.download", "failure", { fileName: req.params.fileName, message: error.message });
    res.status(400).json({ code: 0, msg: error.message, data: null });
  }
});
...
app.get("/api/payment/jl102", (req, res) => table(res, req, engine.jl102TransferRows({ periodId: queryNumber(req, "periodId") || queryNumber(req, "gatherId"), sectionId: queryNumber(req, "sectionId") })));
app.get("/api/payment/jl103", (req, res) => table(res, req, engine.jl103ProgressRows({ periodId: queryNumber(req, "periodId") || queryNumber(req, "gatherId"), sectionId: queryNumber(req, "sectionId") })));
...
app.get("/api/payment/jl108_raw_material", (req, res) => table(res, req, engine.jl108RawMaterialDetailRows({ periodId: queryNumber(req, "periodId") || queryNumber(req, "gatherId"), sectionId: queryNumber(req, "sectionId") })));
app.get("/api/payment/jl112", (req, res) => table(res, req, engine.jl112QuantityCompilationRows({ periodId: queryNumber(req, "periodId") || queryNumber(req, "gatherId"), sectionId: queryNumber(req, "sectionId") })));
...
app.get("/api/payment/jl115", (req, res) => operationOk(res, engine.jl115MobilizationAdvanceCertificate({ periodId: queryNumber(req, "periodId") || queryNumber(req, "gatherId"), sectionId: queryNumber(req, "sectionId") })));
