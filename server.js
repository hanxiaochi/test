...
function internationalCertificateErrorStatus(error) {
  ...
  if (/already|idempotency key/.test(error.message)) return 409;
  return 400;
}
...
function internationalSettingsHtml(req) {
  ...
  const certificateRows = certificates.map((item) => {
    ...
    return `<tr data-certificate-id="${htmlEscape(item.id)}">
      ...
      <td>V${item.settingsVersion} / S${item.settingsSchemaVersion}</td><td>${htmlEscape(item.issuedBy)}<br>${htmlEscape(item.issuedAt)}</td>
    ...
    <div class="core-shell">
      ...
      <div class="core-grid">
        ...
        <div class="core-panel">
          ...
          <div class="layui-form-item"><label class="layui-form-label">${htmlEscape(t("international.periodStart"))}</label><div class="layui-input-block"><input class="layui-input" id="fidic-period-start" type="date" value="${monthStart}" required></div></div>
          <div class="layui-form-item"><label class="layui-form-label">${htmlEscape(t("international.periodEnd"))}</label><div class="layui-input-block"><input class="layui-input" id="fidic-period-end" type="date" value="${currentDate}" required></div></div>
      ...
      <div class="core-panel" style="margin-top:12px;overflow:auto;">
        ...
        <table class="layui-table" lay-size="sm"><thead><tr><th>${htmlEscape(t("international.certificateNo"))}</th><th>${htmlEscape(t("international.periodStart"))} / ${htmlEscape(t("international.periodEnd"))}</th><th>${htmlEscape(t("international.status"))}</th><th>Net certified</th><th>${htmlEscape(t("international.settingsTrace"))}</th><th>${htmlEscape(t("international.issuedBy"))}</th><th>SHA-256</th><th>${htmlEscape(t("international.action"))}</th></tr></thead><tbody id="international-certificate-register">${certificateRows || `<tr><td colspan="8" class="core-empty">${htmlEscape(t("international.noCertificates"))}</td></tr>`}</tbody></table>
      </div>
    </div>
    <script>(function(){
      ...
      function certificateInput(){return {lines:JSON.parse(root.querySelector('#fidic-lines').value||'[]'),previousRetention:root.querySelector('#fidic-previous-retention').value,retentionRelease:root.querySelector('#fidic-retention-release').value,priceAdjustment:JSON.parse(root.querySelector('#fidic-price-adjustment').value||'{}')}}
