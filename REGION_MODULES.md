...
1. 在 `lib/regions/packs/` 新建地区包，声明唯一 ID、语义化版本、依赖、能力、前端资源和后端路由。
...
当前版本已经完成统一注册、前端菜单/页面暴露、后端路由阻断、能力和工作流选择。原有大型 `server.js` 中的部分页面渲染函数仍将逐步迁移到包自有目录；迁移期间注册表是唯一对外暴露边界，每次搬迁必须保持接口和计算回归不变。
