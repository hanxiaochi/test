...
class RuleStore {
  ...
  activate(options = {}) {
    ...
    const projectId = options.projectId === undefined ? null : String(options.projectId);
    const target = tenantId === null
      ...
      : projectId === null
        ...
        : this.db.prepare("SELECT * FROM calculation_rule_versions WHERE id = ? AND tenant_id = ? AND project_id = ?").get(id, tenantId, projectId);
