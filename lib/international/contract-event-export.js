...
const NOTICE_REPORT_TRANSLATIONS = Object.freeze({
  "en-US": { occurrenceDate: "Occurrence date", noticeDeadline: "Notice deadline", noticeElapsed: "Elapsed days", noticeStatus: "Notice status", timely: "Timely", late: "Late", notApplicable: "Not applicable", lateNoticeReason: "Late notice reason", settingsVersion: "Notice settings version", settingsSchemaVersion: "Notice settings schema", settingsChecksum: "Notice settings SHA-256" },
  "zh-CN": { occurrenceDate: "事件发生日期", noticeDeadline: "通知截止日期", noticeElapsed: "经过天数", noticeStatus: "通知时效", timely: "及时", late: "逾期", notApplicable: "不适用", lateNoticeReason: "迟报原因", settingsVersion: "通知参数版本", settingsSchemaVersion: "通知参数结构版本", settingsChecksum: "通知参数 SHA-256" },
  "es-ES": { occurrenceDate: "Fecha del evento", noticeDeadline: "Fecha límite del aviso", noticeElapsed: "Días transcurridos", noticeStatus: "Estado del aviso", timely: "En plazo", late: "Tardío", notApplicable: "No aplicable", lateNoticeReason: "Motivo del aviso tardío", settingsVersion: "Versión de parámetros del aviso", settingsSchemaVersion: "Esquema de parámetros del aviso", settingsChecksum: "SHA-256 de parámetros del aviso" },
  "fr-FR": { occurrenceDate: "Date de l'événement", noticeDeadline: "Date limite de notification", noticeElapsed: "Jours écoulés", noticeStatus: "Statut de notification", timely: "Dans les délais", late: "Tardive", notApplicable: "Sans objet", lateNoticeReason: "Motif de notification tardive", settingsVersion: "Version des paramètres de notification", settingsSchemaVersion: "Schéma des paramètres de notification", settingsChecksum: "SHA-256 des paramètres de notification" },
  "pt-BR": { occurrenceDate: "Data do evento", noticeDeadline: "Prazo da notificação", noticeElapsed: "Dias decorridos", noticeStatus: "Status da notificação", timely: "No prazo", late: "Atrasada", notApplicable: "Não aplicável", lateNoticeReason: "Motivo da notificação tardia", settingsVersion: "Versão dos parâmetros de notificação", settingsSchemaVersion: "Esquema dos parâmetros de notificação", settingsChecksum: "SHA-256 dos parâmetros de notificação" },
  "ar-SA": { occurrenceDate: "تاريخ وقوع الحدث", noticeDeadline: "الموعد النهائي للإشعار", noticeElapsed: "الأيام المنقضية", noticeStatus: "حالة الإشعار", timely: "ضمن المهلة", late: "متأخر", notApplicable: "غير منطبق", lateNoticeReason: "سبب تأخر الإشعار", settingsVersion: "إصدار إعدادات الإشعار", settingsSchemaVersion: "إصدار بنية إعدادات الإشعار", settingsChecksum: "SHA-256 لإعدادات الإشعار" }
});
Object.entries(NOTICE_REPORT_TRANSLATIONS).forEach(([locale, labels]) => Object.assign(REPORT_TRANSLATIONS[locale], labels));
...
function contractEventModel(source, options = {}) {
  ...
  const notice = record.request.noticeAssessment;
  const noticeRows = notice ? [
    [t("occurrenceDate"), record.request.occurredDate], [t("noticeDeadline"), notice.deadlineDate || "-"],
    [t("noticeElapsed"), notice.elapsedDays], [t("noticeStatus"), t(notice.status === "not_applicable" ? "notApplicable" : notice.status)],
    [t("lateNoticeReason"), record.request.lateNoticeReason || "-"]
  ] : [];
  const noticeIntegrityRows = notice ? [
    [t("settingsVersion"), notice.settingsVersion], [t("settingsSchemaVersion"), notice.settingsSchemaVersion], [t("settingsChecksum"), notice.settingsChecksum]
  ] : [];
  return {
    ...
    eventRows: [
      ...
      [t("subject"), record.request.title], [t("noticeDate"), record.request.noticeDate], ...noticeRows, [t("currency"), record.request.currency],
    ...
    integrityRows: [
      ...
      [t("evidenceManifest"), JSON.stringify(record.evidenceManifest || [])], ...noticeIntegrityRows
    ]
  };
}
