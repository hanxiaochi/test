...
function eventReference(line, index) {
...
function referencedAmount(request, eventId) {
  return requestLines(request).reduce((sum, line, index) => {
    const reference = eventReference(line, index);
...
function eventUsage(state, eventId, options = {}) {
  ...
  applicationRows(state).forEach((application) => {
    ...
    const amount = referencedAmount(application.request, eventId);
  ...
  certificateRows.forEach((certificate) => {
    ...
    issued = issued.plus(referencedAmount(certificate, eventId));
  });
...
function validateCertificateEventAllocations(state, request, options = {}) {
  ...
  const references = lines.map(eventReference).filter(Boolean);
  const seen = new Set();
