...
function normalizeLines(lines, options) {
  ...
  return lines.map((line, index) => {
    ...
    const contractEventId = String(line.contractEventId || "").trim();
    const contractEventDecisionChecksum = String(line.contractEventDecisionChecksum || "").trim().toLowerCase();
    if (contractEventId || contractEventDecisionChecksum) {
      if (!contractEventId || contractEventId.length > 64) throw new Error(`line ${code} contract event id is invalid`);
      if (!/^[a-f0-9]{64}$/.test(contractEventDecisionChecksum)) throw new Error(`line ${code} contract event decision checksum is invalid`);
    }
    return { code, description: String(line.description || code), category, direction, currency, amount, exchangeRate, baseAmount, generated: line.generated === true, contractEventId, contractEventDecisionChecksum };
  });
}
...
function calculateCertificate(input = {}, configuration = {}) {
  ...
  return {
    ...
    lines: lines.map((line) => ({
      ...
      ...(line.contractEventId ? { contractEventId: line.contractEventId, contractEventDecisionChecksum: line.contractEventDecisionChecksum } : {})
    })),
