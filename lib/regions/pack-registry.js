...
function normalizePack(value) {
  ...
  return {
    ...
    frontend: { topMenuIds: integerList(frontend.topMenuIds || [], `region pack ${id} top menu ids`), resourceIds: integerList(frontend.resourceIds || [], `region pack ${id} resource ids`), pages },
    backend: { exactRoutes: stringList(backend.exactRoutes || [], `region pack ${id} exact routes`).map((item) => route(item, `region pack ${id} exact route`)), routePrefixes: stringList(backend.routePrefixes || [], `region pack ${id} route prefixes`).map((item) => route(item, `region pack ${id} route prefix`)) }
  };
}
