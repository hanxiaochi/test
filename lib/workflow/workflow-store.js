...
function certificateApplicationDefinition() {
  return normalizeDefinition({
    initialState: "draft",
    states: [
      { code: "draft", label: "草稿" },
      { code: "pending", label: "待审核" },
      { code: "approved", label: "已批准", terminal: true },
      { code: "returned", label: "已退回", terminal: true }
    ],
    transitions: [
      { action: "submit", label: "提交审核", from: ["draft", "returned"], to: "pending", permission: "international:submit" },
      { action: "approve", label: "批准申请", from: ["pending"], to: "approved", permission: "international:review", requireRemark: true },
      { action: "return", label: "退回修改", from: ["pending"], to: "returned", permission: "international:review", requireRemark: true }
    ]
  });
}

function contractEventDefinition() {
  return normalizeDefinition({
    initialState: "draft",
    states: [
      { code: "draft", label: "草稿" },
    ...
    transitions: [
      { action: "submit", label: "提交审核", from: ["draft", "returned"], to: "pending", permission: "international:submit" },
      { action: "approve", label: "批准合同事件", from: ["pending"], to: "approved", permission: "international:review", requireRemark: true },
      { action: "return", label: "退回合同事件", from: ["pending"], to: "returned", permission: "international:review", requireRemark: true }
    ]
  });
}
...
module.exports = { WorkflowError, WorkflowStore, certificateApplicationDefinition, contractEventDefinition, defaultDefinition, definitionChecksum, definitionPayload, normalizeDefinition };
