...
class WorkflowStore {
  ...
  transition(options = {}) {
    ...
    const module = code(options.module, "Workflow module");
    const businessId = text(options.businessId, "Business id", 100);
    const action = code(options.action, "Workflow action");
    ...
    const expectedRevision = options.expectedRevision === undefined || options.expectedRevision === null || options.expectedRevision === "" ? null : Number(options.expectedRevision);
    ...
    return this.transaction(() => {
      let row = this.db.prepare("SELECT * FROM workflow_instances WHERE tenant_id=? AND project_id=? AND module=? AND business_id=?").get(tenantId, projectId, module, businessId);
      if (!row) {
        ...
        const now = this.nowIso();
        const initialState = this.resolveInitialState(active.definition, options.currentStateLabel);
        const inserted = this.db.prepare(`INSERT INTO workflow_instances
          ...
          VALUES(?,?,?,?,?,?,?,0,?,?)`).run(tenantId, projectId, module, businessId, String(options.businessNo || "").slice(0, 200), active.id, initialState, now, now);
        row = this.db.prepare("SELECT * FROM workflow_instances WHERE id=?").get(Number(inserted.lastInsertRowid));
      }
      if (expectedRevision !== null && Number(row.revision) !== expectedRevision) {
        throw new WorkflowError("Workflow has been updated; refresh and retry", "WORKFLOW_REVISION_CONFLICT", 409, { expectedRevision, actualRevision: Number(row.revision) });
      }
      const version = this.getDefinition(Number(row.definition_id));
      const transition = version.definition.transitions.find((item) => item.action === action && item.from.includes(row.current_state));
      if (!transition) throw new WorkflowError(`Action ${action} is not allowed from ${row.current_state}`, "WORKFLOW_TRANSITION_NOT_ALLOWED", 409, { currentState: row.current_state });
      if (!hasPermission(permissions, transition.permission)) throw new WorkflowError("No permission for workflow action", "WORKFLOW_PERMISSION_DENIED", 403, { requiredPermission: transition.permission });
      const remark = String(options.remark || "").trim().slice(0, 2000);
      if (transition.requireRemark && !remark) throw new WorkflowError("Remark is required for this workflow action", "WORKFLOW_REMARK_REQUIRED", 400);
      const revision = Number(row.revision) + 1;
      const now = this.nowIso();
      this.db.prepare("UPDATE workflow_instances SET current_state=?,revision=?,business_no=?,updated_at=? WHERE id=?").run(transition.to, revision, String(options.businessNo || row.business_no || "").slice(0, 200), now, Number(row.id));
      const event = this.db.prepare(`INSERT INTO workflow_events
        ...
        VALUES(?,?,?,?,?,?,?,?,?)`).run(Number(row.id), revision, action, row.current_state, transition.to, options.actorUserId || null, account, remark, now);
      const result = { instance: this.getInstance(tenantId, projectId, module, businessId), eventId: Number(event.lastInsertRowid), action, fromState: row.current_state, toState: transition.to, toStateLabel: version.definition.states.find((item) => item.code === transition.to).label };
      if (typeof options.applyState === "function") options.applyState(result);
      return result;
    });
  }
