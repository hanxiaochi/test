...
function createTextPdf(title, sourceLines, options = {}) {
  const reportTitle = String(title || "计量支付报表").trim();
  const generatedAt = options.generatedAt ? new Date(options.generatedAt) : new Date();
  const lines = (Array.isArray(sourceLines) ? sourceLines : []).map((line) => String(line ?? "").trim());
  return new Promise((resolve, reject) => {
    const chunks = [];
    const doc = new PDFDocument({
      size: "A4",
      margins: { top: 38, right: 42, bottom: 42, left: 42 },
      bufferPages: true,
      info: { Title: reportTitle, Author: "Engineering Payment Platform", Subject: "JL计量支付报表导出", CreationDate: generatedAt }
    });
    doc.on("data", (chunk) => chunks.push(chunk));
    doc.on("error", reject);
    doc.on("end", () => resolve(Buffer.concat(chunks)));
    ...
    const drawHeader = (continuation = false) => {
      doc.fillColor("#172033").fontSize(continuation ? 11 : 17).text(continuation ? `${reportTitle}（续）` : reportTitle, { align: "center" });
      doc.moveDown(0.25).fillColor("#64748B").fontSize(7.5).text(`生成时间：${generatedAt.toISOString()}  内容行数：${lines.filter(Boolean).length}`, { align: "right" });
      doc.moveDown(0.75);
    };
    ...
    lines.forEach((line, index) => {
      ...
      if (line.startsWith("【") && line.endsWith("】")) {
        ensureSpace(30);
        const y = doc.y;
        doc.roundedRect(doc.page.margins.left, y, contentWidth(), 24, 3).fill("#CCFBF1");
        doc.fillColor("#0F766E").fontSize(10).text(line, doc.page.margins.left + 8, y + 6, { width: contentWidth() - 16, lineBreak: false, ellipsis: true });
        doc.x = doc.page.margins.left;
        doc.y = y + 31;
        return;
      }
      doc.fontSize(index < 2 ? 9 : 8.2);
      const paragraphHeight = doc.heightOfString(line, { width: contentWidth() - 12, lineGap: 2 });
      ensureSpace(Math.min(paragraphHeight + 13, pageBottom() - doc.page.margins.top));
      const y = doc.y;
      doc.fillColor(index < 2 ? "#475569" : "#172033").text(line, doc.page.margins.left + 6, y, { width: contentWidth() - 12, lineGap: 2 });
      doc.x = doc.page.margins.left;
      doc.moveDown(0.35);
      doc.strokeColor("#E2E8F0").moveTo(doc.page.margins.left, doc.y).lineTo(doc.page.margins.left + contentWidth(), doc.y).stroke();
      doc.moveDown(0.45);
    });
    ...
    for (let index = 0; index < pages.count; index += 1) {
      doc.switchToPage(pages.start + index);
      doc.fillColor("#64748B").fontSize(7).text(`第 ${index + 1} / ${pages.count} 页`, doc.page.margins.left, doc.page.height - doc.page.margins.bottom - 18, { width: contentWidth(), align: "right", lineBreak: false });
    }
    doc.end();
  });
}
...
module.exports = { COLUMNS, FINANCIAL_COLUMNS, MONEY_KEYS, PAYMENT_COLUMNS, createDocx, createPdf, createTextPdf, createXlsx, normalizeRows, number, pdfLines, reportModel, summarize };
