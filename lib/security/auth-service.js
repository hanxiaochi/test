...
const { deploymentSecurityPolicy } = require("./deployment-security");
...
const deployment = deploymentSecurityPolicy(process.env);
...
const bootstrap = store.bootstrap({
  ...
  requireStrongPassword: deployment.requireStrongBootstrap
});

if (deployment.requireStrongBootstrap && store.passwordMatches(bootstrap.userId, "000000")) {
  store.close();
  throw new Error("External or production startup refused: replace the insecure bootstrap administrator password first");
}

module.exports = { bootstrap, deployment, securityFile, store };
