...
const { AccountSecurityControls } = require("./account-security");
...
function auditDate(value, label) {
...
function auditText(value, maxLength = 200) {
...
class SecurityStore {
  constructor(file, options = {}) {
    ...
    this.migrate();
    this.accountSecurity = new AccountSecurityControls(this.db, { now: this.now, policy: options.policy, env: options.env });
  }
  ...
  migrate() {
    ...
    this.db.prepare("INSERT OR IGNORE INTO security_migrations(version, applied_at) VALUES(4, ?)").run(this.nowIso());
    this.db.prepare("INSERT OR IGNORE INTO security_migrations(version, applied_at) VALUES(5, ?)").run(this.nowIso());
    this.db.prepare("SELECT id FROM tenants").all().forEach((tenant) => this.ensureBuiltinAuthorization(tenant.id));
  }
  ...
  ensureBuiltinAuthorization(tenantId) {
    const permissions = [
      ["*", "全部权限"], ["data:read", "查看业务数据"], ["data:write", "修改业务数据"], ["admin:access", "访问管理后台"], ["admin:users", "管理用户与角色"],
      ["international:read", "查看国际付款证书"], ["international:calculate", "试算国际付款证书"], ["international:export", "导出国际付款证书"],
      ["international:submit", "提交国际付款证书申请"], ["international:review", "审核国际付款证书申请"],
      ["international:issue", "签发国际付款证书"], ["international:void", "作废国际付款证书"]
    ];
    permissions.forEach(([code, name]) => this.db.prepare("INSERT OR IGNORE INTO permissions(code, name) VALUES(?, ?)").run(code, name));
    [["admin", "系统管理员"], ["editor", "业务编辑者"], ["viewer", "只读用户"], ["certificate_approver", "国际证书签发人"]]
      .forEach(([code, name]) => this.db.prepare("INSERT OR IGNORE INTO roles(tenant_id, code, name, built_in) VALUES(?, ?, ?, 1)").run(String(tenantId), code, name));
    const roleId = (code) => this.db.prepare("SELECT id FROM roles WHERE tenant_id = ? AND code = ?").get(String(tenantId), code).id;
    const grants = {
      admin: ["*"],
      editor: ["data:read", "data:write", "international:read", "international:calculate", "international:export", "international:submit"],
      viewer: ["data:read", "international:read", "international:calculate", "international:export"],
      certificate_approver: ["data:read", "international:read", "international:calculate", "international:export", "international:review", "international:issue", "international:void"]
    };
    Object.entries(grants).forEach(([roleCode, codes]) => codes.forEach((permission) => {
      this.db.prepare("INSERT OR IGNORE INTO role_permissions(role_id, permission_code) VALUES(?, ?)").run(roleId(roleCode), permission);
    }));
  }
  ...
  bootstrap(options = {}) {
    ...
    try {
      ...
      this.ensureBuiltinAuthorization(tenantId);
      ...
      if (!user) {
        ...
        const result = this.db.prepare(`
          INSERT INTO users(tenant_id, account, display_name, password_hash, status, must_change_password, password_changed_at, created_at, updated_at)
          VALUES(?, ?, ?, ?, 'active', 1, ?, ?, ?)
        `).run(tenantId, account, displayName, passwordHash, now, now, now);
        user = { id: Number(result.lastInsertRowid) };
        this.accountSecurity.recordPassword(user.id, passwordHash, now);
        created = true;
      }
  ...
  authenticate(options = {}) {
    ...
    const locked = user && this.accountSecurity.isLocked(user);
    if (!user || user.status !== "active" || user.tenant_status !== "active" || locked || !passwordOk) {
      const lockState = user && user.status === "active" && user.tenant_status === "active" && !locked && !passwordOk
        ? this.accountSecurity.recordLoginFailure(user.id)
        : { lockedUntil: user && user.locked_until };
      this.audit({
        tenantId,
        ...
        details: { account, reason: locked ? "account-locked" : "credentials", lockedUntil: lockState.lockedUntil || null }
      });
      return null;
    }
    this.accountSecurity.recordLoginSuccess(user.id);
    const token = createSessionToken();
    const createdAt = this.nowIso();
    const expiresAt = this.accountSecurity.sessionExpiresAt(options.remember);
    this.db.exec("BEGIN IMMEDIATE");
    try {
      this.db.prepare(`
        INSERT INTO sessions(token_hash, user_id, created_at, expires_at, last_seen_at, ip_address, user_agent)
        VALUES(?, ?, ?, ?, ?, ?, ?)
      `).run(hashSessionToken(token), user.id, createdAt, expiresAt, createdAt, String(options.ipAddress || ""), String(options.userAgent || ""));
      this.audit({
        tenantId,
  ...
  passwordMatches(userId, password) {
    const user = this.db.prepare("SELECT password_hash FROM users WHERE id = ?").get(Number(userId));
  ...
  changePassword(options = {}) {
    ...
    this.accountSecurity.assertPasswordNotReused(userId, options.newPassword);
    const now = this.nowIso();
    const passwordHash = hashPassword(options.newPassword);
    this.db.exec("BEGIN IMMEDIATE");
    try {
      this.db.prepare("UPDATE users SET password_hash = ?, must_change_password = 0, password_changed_at = ?, updated_at = ? WHERE id = ?")
        .run(passwordHash, now, now, userId);
      this.accountSecurity.recordPassword(userId, passwordHash, now);
  ...
  userView(user) {
    return {
      ...
      status: user.status,
      mustChangePassword: Boolean(user.must_change_password) || this.accountSecurity.passwordExpired(user),
      passwordExpired: this.accountSecurity.passwordExpired(user),
  ...
  getSession(token) {
    ...
    const row = this.db.prepare(`
      SELECT s.token_hash, s.created_at AS session_created_at, s.last_seen_at, s.expires_at, s.revoked_at, u.*,
    ...
    if (!this.accountSecurity.sessionAllowed(row)) return null;
    this.accountSecurity.touchSession(row.token_hash);
  ...
  securityPosture(tenantId = "default") {
    const databaseOk = this.db.prepare("PRAGMA quick_check").get().quick_check === "ok";
    const usersWithoutRoles = Number(this.db.prepare(`
      SELECT COUNT(*) AS count FROM users u
      WHERE u.tenant_id = ? AND NOT EXISTS (SELECT 1 FROM user_roles ur WHERE ur.user_id = u.id)
    `).get(String(tenantId)).count);
    const activeAdministrators = Number(this.db.prepare(`
      SELECT COUNT(DISTINCT u.id) AS count FROM users u
      JOIN user_roles ur ON ur.user_id = u.id JOIN roles r ON r.id = ur.role_id
      WHERE u.tenant_id = ? AND u.status = 'active' AND r.code = 'admin'
    `).get(String(tenantId)).count);
    const lockedUsers = Number(this.db.prepare("SELECT COUNT(*) AS count FROM users WHERE tenant_id = ? AND locked_until > ?")
      .get(String(tenantId), this.nowIso()).count);
    return { databaseOk, usersWithoutRoles, activeAdministrators, lockedUsers };
  }
  ...
  activeAdminCount(tenantId = "default") {
  ...
  setUserStatus(options = {}) {
    ...
    if (!user) throw new Error("User does not exist");
    if (status === "disabled" && user.status === "active" && this.isAdminUser(userId) && this.activeAdminCount(user.tenant_id) <= 1) {
      throw new Error("不能禁用最后一个有效系统管理员");
    }
    this.db.exec("BEGIN IMMEDIATE");
    try {
  ...
  resetUserPassword(options = {}) {
    ...
    const policy = validatePasswordPolicy(options.password);
    if (!policy.ok) throw new Error(policy.failures.join("；"));
    this.accountSecurity.assertPasswordNotReused(userId, options.password);
    const now = this.nowIso();
    const passwordHash = hashPassword(options.password);
    this.db.exec("BEGIN IMMEDIATE");
    try {
      this.db.prepare("UPDATE users SET password_hash = ?, must_change_password = 1, password_changed_at = ?, updated_at = ? WHERE id = ?")
        .run(passwordHash, now, now, userId);
      this.accountSecurity.recordPassword(userId, passwordHash, now);
      this.db.prepare("UPDATE sessions SET revoked_at = ? WHERE user_id = ? AND revoked_at IS NULL").run(now, userId);
      this.audit({
  ...
  setUserRoles(options = {}) {
    ...
    const user = this.db.prepare("SELECT * FROM users WHERE id = ? AND tenant_id = ?").get(userId, tenantId);
    if (!user) throw new Error("User does not exist");
    if (this.isAdminUser(userId) && !roleCodes.includes("admin") && user.status === "active" && this.activeAdminCount(tenantId) <= 1) {
      throw new Error("不能移除最后一个有效系统管理员的管理员角色");
    }
  ...
  unlockUser(options = {}) {
    ...
    const user = this.db.prepare("SELECT * FROM users WHERE id = ? AND tenant_id = ?").get(userId, tenantId);
    if (!user) throw new Error("User does not exist");
    this.accountSecurity.unlock(userId);
    this.audit({ tenantId, userId: options.actorUserId, action: "user.unlock", result: "success", targetType: "user", targetId: String(userId) });
    return this.userView(this.db.prepare("SELECT * FROM users WHERE id = ?").get(userId));
  }

  auditRows(limit = 100, tenantId = null) {
    return this.queryAudit({ tenantId, limit }, { maxLimit: 1000 }).rows;
  }

  queryAudit(options = {}, controls = {}) {
    const maxLimit = Math.max(1, Math.min(10000, Number(controls.maxLimit) || 200));
    const page = Math.max(1, Math.min(1000000, Number(options.page) || 1));
    const limit = Math.max(1, Math.min(maxLimit, Number(options.limit) || 30));
    const where = [];
    const params = [];
    ...
    [["action", "action"], ["result", "result"], ["targetType", "target_type"]].forEach(([key, column]) => {
      const value = auditText(options[key], 100);
      if (value) {
        where.push(`${column} = ?`);
        params.push(value);
      }
    ...
    if (options.userId !== undefined && options.userId !== null && String(options.userId).trim() !== "") {
      const userId = Number(options.userId);
      if (!Number.isInteger(userId) || userId <= 0) throw new Error("userId is invalid");
      where.push("user_id = ?");
      params.push(userId);
    }
    const from = auditDate(options.from, "from");
    const to = auditDate(options.to, "to");
    ...
    const keyword = auditText(options.keyword);
    if (keyword) {
      const pattern = `%${escapeLike(keyword)}%`;
      where.push("(action LIKE ? ESCAPE '\\' OR target_id LIKE ? ESCAPE '\\' OR ip_address LIKE ? ESCAPE '\\' OR details_json LIKE ? ESCAPE '\\')");
      params.push(pattern, pattern, pattern, pattern);
    }
    const clause = where.length ? ` WHERE ${where.join(" AND ")}` : "";
    const total = Number(this.db.prepare(`SELECT COUNT(*) AS count FROM security_audit${clause}`).get(...params).count);
    const rows = this.db.prepare(`SELECT * FROM security_audit${clause} ORDER BY id DESC LIMIT ? OFFSET ?`)
      .all(...params, limit, (page - 1) * limit);
    return { rows, total, page, limit, pages: Math.max(1, Math.ceil(total / limit)) };
  }

  auditExportRows(options = {}) {
    return this.queryAudit({ ...options, page: 1, limit: 10000 }, { maxLimit: 10000 }).rows;
  }

  pruneAudit(options = {}) {
    const tenantId = auditText(options.tenantId, 100);
    if (!tenantId) throw new Error("tenantId is required");
    const before = auditDate(options.before, "before");
    if (!before) throw new Error("before is required");
    this.db.exec("BEGIN IMMEDIATE");
    try {
      const deleted = Number(this.db.prepare("DELETE FROM security_audit WHERE tenant_id = ? AND created_at < ?").run(tenantId, before).changes);
      this.audit({
        tenantId,
        userId: options.actorUserId,
        action: "security.audit.retention",
        result: "success",
        targetType: "tenant",
        targetId: tenantId,
        ipAddress: options.ipAddress,
      ...
      this.db.exec("COMMIT");
      return { tenantId, before, deleted };
    } catch (error) {
      this.db.exec("ROLLBACK");
      throw error;
    }
