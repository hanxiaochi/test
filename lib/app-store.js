...
function loadScope(tenantId, projectId = "1", template = {}) {
  ...
  return sqliteStoreFor(key, projectKey).initialize(seed, { actor: "system", action: "scope-initialize", checkpoint: true });
}
...
function load(fallback = {}) {
  ...
  return sqliteStoreFor("default").initialize(seed, { action: "migrate-json" });
}
...
function save(state, metadata = {}) {
  ...
  return sqliteStoreFor(tenantId, projectId).save(state, metadata);
}
