...
APP_PASSWORD_MAX_AGE_DAYS 密码有效期天数，安全范围30-365，默认90
APP_PASSWORD_HISTORY_COUNT 禁止复用的最近密码次数，安全范围3-20，默认5
APP_ACCOUNT_LOCK_ATTEMPTS 账号级连续失败锁定阈值，安全范围3-10，默认5
APP_ACCOUNT_LOCK_MINUTES  账号锁定分钟数，安全范围5-1440，默认30
APP_SESSION_IDLE_MINUTES  会话闲置超时分钟数，安全范围5-240，默认30
APP_SESSION_MAX_HOURS     会话绝对最长小时数，安全范围1-24，默认8
APP_AUDIT_RETENTION_DAYS  安全审计基线留存天数，安全范围180-3650，默认180
...
首次初始化的管理员会被强制进入密码修改页。新密码至少 10 位，并同时包含字母、数字和特殊字符；默认不得复用最近 5 次密码且 90 天到期。修改成功后全部已有会话会立即失效，必须使用新密码重新登录。
...
账号本身连续 5 次密码错误后默认锁定 30 分钟，与身份限流共同生效。会话默认闲置 30 分钟失效且绝对时长不超过 8 小时。执行 `npm run verify:security-baseline` 可检查产品技术控制；正式等保边界和非代码要求见 `SECURITY_BASELINE.md`。
